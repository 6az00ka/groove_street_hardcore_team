        ...
        while i < numV:
            ...
            while j < numV:
                ...
                # Проверка на наличие ребра
                    ...
                    # Запись начальной точки ребра в массив
                    chainGraph[0][count] = i+1
                    # Запись конечной точки ребра в массив
                    chainGraph[1][count] = j+1
                    # Заполнение используемых ребер контрастными значениями
                    chain[i][j] = -1
                    # Заполнение используемых ребер контрастными значениями
                    chain[j][i] = -1
                    i = j  # Переход к конечной точке ребра
                    # Генерация случайного значения j для оптимального построения цепи
                    j = random.randint(0, numV-1)
                ...
                # Проверка на контрастные значения
                    ...
                    for z in range(0, numV-1):  # Цикл для проверки всей строки массива
                        # Если значения контрастные, то к счетчику прибавляется 1, когда счетчик достигнет максимального значения, общий цикл завершится
                        if chain[i][z] == -1 or chain[i][z] == 0:
        ...
        while i < numV:
            ...
            while j < numV:
                ...
                # Проверка на наличие ребра
                    ...
                    # Запись начальной точки ребра в массив
                    simpleChainGraph[0][count] = i+1
                    # Запись конечной точки ребра в массив
                    simpleChainGraph[1][count] = j+1
                    for z in range(0, numV-1):
                        # Заполнение всех остальных путей из вершины нулями, т.к. ими больше нельзя пользоваться
                        simpleChain[i][z] = 0
                        simpleChain[z][i] = 0
                    simpleChain[i][j] = -1
                    simpleChain[j][i] = -1
                ...
                # Проверка на контрастные значения
                    ...
                    for z in range(0, numV-1):  # Цикл для проверки всей строки массива
                        # Если значения контрастные, то к счетчику прибавляется 1, когда счетчик достигнет максимального значения, общий цикл завершится
                        if simpleChain[i][z] == -1 or simpleChain[i][z] == 0:
                            count1 += 1  # Перевод счетчика на +1
                            if count1 >= numV-1:  # Проверка счетчика на максимальное значение
                                count = max
                    count1 = 0  # Обнуление счетчика в случае неудачи цикла
        ...
        while i < numV:
            ...
            while j < numV:
                ...
                # Проверка на наличие ребра
                    ...
                    # Запись начальной точки ребра в массив
                    orientedSimpleWayGraph[0][count] = i+1
                    # Запись конечной точки ребра в массив
                    orientedSimpleWayGraph[1][count] = j+1
                    # Заполнение используемых ребер контрастными значениями
                    orientedSimpleWay[i][j] = -1
                    # Заполнение используемых ребер контрастными значениями
                    orientedSimpleWay[j][i] = -1
                    i = j  # Переход к конечной точке ребра
                    # Генерация случайного значения j для оптимального построения цепи
                    j = random.randint(0, numV-1)
                    count += 1  # Перевод счетчика на +1
                ...
                # Проверка на контрастные значения
                    ...
                    for z in range(0, numV-1):  # Цикл для проверки всей строки массива
                        # Если значения контрастные, то к счетчику прибавляется 1, когда счетчик достигнет максимального значения, общий цикл завершится
                        if orientedSimpleWay[i][z] == -1 or orientedSimpleWay[i][z] == 0:
                            count1 += 1  # Перевод счетчика на +1
                            if count1 >= numV-1:  # Проверка счетчика на максимальное значение
                                count = max
                    count1 = 0  # Обнуление счетчика в случае неудачи цикла
...
def main():
    global arraySmejnost, arrayIncedent, numV, numE, orientedArraySmejnost, orientedArrayIncedent
    numV = 6
    numE = 12
    ...
    oriented_simple_way_graph_array(
        orientedSimpleWay, orientedSimpleWayGraph, max, numV)
